...
export default {
  ...
  methods:{
    async getInfo(){
        // 在切换tab的时候，最后一次回来的ajax，就填充谁的数据，也就是 ajax 请求有快慢，不可能因为用户点击那个就立马变成那个请求完成优先。
        // 对应路由的接口
        ...
        // 闭包把当前数据请求的 this.activeName 状态保存在 activeName 中
        (async (activeName) => {
          let data = await getOtherInfo[this.activeName]({userId: this.userInfo.userId});
          // 这个时候得等数据异步请求后再赋值，所以 activeName 暂时不受到 this.activeName 的影响
          this.queen[activeName] = data.list;  // this.queen.works = 作品的数据
          // 最后的 this.activeName 不一定就是现在数据正在请求的，所以判断后可以提高程序性能
